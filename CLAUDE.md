...
- **Territory polygons**: OpenHistoricalMap (CC0) — fetched live as vector tiles from `vtiles.openhistoricalmap.org`; no polygon geometry stored in our DB.

## Territory model
Territories are rendered directly from OHM vector tiles. Coloring joins tile features to our polities via an `osm_id → wikidata_qid` lookup served by `/api/ohm-qid-map` (proxied Overpass query, 5-min server cache). Mapping overrides (renames, manual links) live in the `ohm_territory_links` table — see `/api/ohm-links*` endpoints in `server/main.py`.

The legacy historical-basemaps `territories` table and `/api/territories*` endpoints were removed in May 2026.

## Wikidata edit architecture
Edits made via the in-app edit UI (dates, locations, descriptions) are submitted **directly to Wikidata** via the Wikidata OAuth API — they do not go into our DB. The DB is populated by re-running the pipeline after Wikidata updates propagate.
...
- **Manual import flow**: In the OHM mapping modal, typing a search query also queries Wikidata live. Results not already in our DB appear under "From Wikipedia" with an **Import** button.
...
- **After import**: the polity is live immediately and colorable in the OHM mapping flow. No `/deploy` needed for it to be selectable.
